...
Field::Field()
{
	...
	int lines = csv->GetLines(); // ècÇÃçsêî
	maps.resize(lines); // mapsÇÃçsêîÇcsvÇ…çáÇÌÇπÇÈ
	for (int y = 0; y < lines; y++) {
		int cols = csv->GetColumns(y); // ÇªÇÃçsÇÃâ°ÇÃêî
		maps[y].resize(cols); // maps[y]ÇÃóÒêîÇcsvÇ…çáÇÌÇπÇÈ
