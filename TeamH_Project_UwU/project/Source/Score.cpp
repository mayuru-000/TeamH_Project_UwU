...
void Score::Draw()
{
	...
	if (score > 0) {
		...
		if (e->getFlash()) {
			DrawFormatStringToHandle(x - (txtWidth / 2), y, GetColor(0, 0, 255), c->textFont[0], "%d", score);
		}
		else {
			DrawFormatStringToHandle(x - (txtWidth / 2), y, GetColor(0, 255, 255), c->textFont[0], "%d", score);
		}
	}
